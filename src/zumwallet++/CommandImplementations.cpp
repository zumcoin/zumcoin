...
///////////////////////////////////////////////
#include <zumwallet++/CommandImplementations.h>
///////////////////////////////////////////////
...
#include <zumwallet++/GetInput.h>
#include <zumwallet++/Menu.h>
#include <zumwallet++/Open.h>
#include <zumwallet++/Sync.h>
#include <zumwallet++/Utilities.h>

void changePassword(const std::shared_ptr<WalletBackend> walletBackend)
{
    /* Check the user knows the current password */
    ZumUtilities::confirmPassword(walletBackend, "Confirm your current password: ");
...
{
    const uint64_t scanHeight = ZumUtilities::getScanHeight();
    ...
    if (!ZumUtilities::confirm("Are you sure?"))
    {
        return;
    }
...
void saveCSV(const std::shared_ptr<WalletBackend> walletBackend)
{
    ...
    for (const auto tx : transactions)
    {
        ...
        csv << ZumUtilities::unixTimeToDate(tx.timestamp) << ","    /* Timestamp */
...
void printOutgoingTransfer(const WalletTypes::Transaction tx)
{
    ...
    if (tx.blockHeight != 0 && tx.timestamp != 0)
    {
        stream << "Block height: " << tx.blockHeight << "\n"
               << "Timestamp: " << ZumUtilities::unixTimeToDate(tx.timestamp) << "\n";
    }
