...
namespace WalletConfig
{
    ...
    const std::string_view addressPrefix = "Zum1";

    /* Your coins 'Ticker', e.g. Monero = XMR, Bitcoin = BTC */
    const std::string ticker = "ZUM";
    ...
    const std::string daemonName = "ZumCoind";

    /* The name to call this wallet */
    const std::string walletName = "zumwallet";

    /* The name of service/walletd, the programmatic rpc interface to a
       wallet */
    const std::string walletdName = "zum-service";

    /* The full name of your crypto */
    const std::string coinName = std::string(CryptoNote::CRYPTONOTE_NAME);

    /* Where can your users contact you for support? E.g. discord */
    const std::string contactLink = "http://discord.zumcoin.org";
    ...
    const uint64_t minimumSend = 100000000;  // 1 ZUMCOIN IS MINIMUM ALLOWED TO BE SPENT
