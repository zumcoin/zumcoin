...
bool Currency::generateGenesisBlock() {
  ...
  genesisBlockTemplate.nonce = 786;
  if (m_testnet) {
...
uint64_t Currency::baseRewardFunction(uint64_t alreadyGeneratedCoins, uint32_t height, uint64_t fee) const {
  ...
  size_t l_mod = 199;
  size_t r_mod = 99;
  uint64_t x = alreadyGeneratedCoins;
  uint32_t y = height;
  uint64_t z = fee;
  uint64_t c =  CryptoNote::parameters::COIN;
  uint64_t j_reward = CryptoNote::parameters::REWARD_JACKPOT;
  uint64_t m_reward = CryptoNote::parameters::REWARD_MEGA_7;
  uint64_t s_reward = CryptoNote::parameters::REWARD_SUPER_7;
  uint64_t l_reward = CryptoNote::parameters::REWARD_LUCKY_7;
  uint64_t min_reward = CryptoNote::parameters::REWARD_MIN;
  uint64_t max_reward = CryptoNote::parameters::REWARD_MAX;
  

  if (height >= CryptoNote::parameters::REWARD_V1_HEIGHT && height < CryptoNote::parameters::REWARD_V2_HEIGHT) {
    uint64_t baseReward = CryptoNote::parameters::REWARD_V1;
    return baseReward;
  }

  if (height >= CryptoNote::parameters::REWARD_V2_HEIGHT) {


      if (height % j == 0) {
            uint64_t baseReward = (((x + y + z) % c) * ((x + y + z) % j_mod));
            if(baseReward < min_reward) { baseReward = min_reward;}
            if(baseReward > j_reward) { baseReward = j_reward;}
            assert(baseReward >= min_reward && baseReward <= j_reward);
            return baseReward;
      }

      if (height % m == 0) {
            uint64_t baseReward = (((x + y + z) % c) * ((x + y + z) % m_mod));
            if(baseReward < min_reward) { baseReward = min_reward;}
            if(baseReward > m_reward) { baseReward = m_reward;}
            assert(baseReward >= min_reward && baseReward <= m_reward);
            return baseReward;
      }

      if (height % s == 0) {
            uint64_t baseReward = (((x + y + z) % c) * ((x + y + z) % s_mod));
            if(baseReward < min_reward) { baseReward = min_reward;}
            if(baseReward > s_reward) { baseReward = s_reward;}
            assert(baseReward >= min_reward && baseReward <= s_reward);
            return baseReward;
      }

      if (height % l == 0) {
            uint64_t baseReward = (((x + y + z) % c) * ((x + y + z) % l_mod));
            if(baseReward < min_reward) { baseReward = min_reward;}
            if(baseReward > l_reward) { baseReward = l_reward;}
            assert(baseReward >= min_reward && baseReward <= l_reward);
            return baseReward;
      }

      uint64_t baseReward = (((x + y + z) % c) * ((x + y + z) % r_mod));
          if(baseReward < min_reward) { baseReward = min_reward;}
          if(baseReward > max_reward) { baseReward = max_reward;}
          assert(baseReward >= min_reward && baseReward <= max_reward);
          return baseReward;
  }
}
...
bool Currency::getBlockReward(uint8_t blockMajorVersion, size_t medianSize, size_t currentBlockSize, uint64_t alreadyGeneratedCoins,
  ...
  uint64_t baseReward = baseRewardFunction(alreadyGeneratedCoins, height, fee);
...
bool Currency::constructMinerTx(uint8_t blockMajorVersion, uint32_t height, size_t medianSize, uint64_t alreadyGeneratedCoins, size_t currentBlockSize,
  ...
  uint64_t currentBlockdifficulty;
