# USAGE: Start zum-service as you usually would
...
from zumcoin import Walletd as Wallet
import time
import argparse
parser = argparse.ArgumentParser()
parser.add_argument("--address", help="zumcoin address to optimize", required=True)
parser.add_argument("--port", type=int, help="Wallet port", required=False, default=19070)
...
def main():
    ...
    while True:
        fuseable = wallet.estimate_fusion(1000000000000000, [args.address])
        print("Outputs fuseable: {}".format(fuseable))
        try:
            print("Sending Fusion")
            fusion_result = wallet.send_fusion_transaction(1000000000000000, 0, [args.address], args.address)
