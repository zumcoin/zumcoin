...
# zumcoin Multi-installer
# a one line clone-and-compile for zumcoin:
#
#     ` $ curl -sL "https://raw.githubusercontent.com/zumcoin/zumcoin/master/scripts/multi_installer.sh" | bash
#
...
_fail() {
    ...
    _colorize red "Please check build.log and if you need help check out the team discord @ 'http://discord.zumcoin.org'" && echo
...
_set_wd() {
    if [ -d "$PWD/.git" ] && [ -f "$PWD/Makefile" ]; then
        ...
        if [ -d "$PWD"/zumcoin ]; then
            read -r -p "${1:-zumcoin directory already exists. Overwrite? [y/N]} " response
            case "$response" in
                [yY][eE][sS|[yY])
                    _colorize red "Overwriting old zumcoin directory" && echo
                    rm -rf "$PWD"/zumcoin
                    ;;
                *)
                    _fail "zumcoin directory already exists. Aborting..."
                    ;;
            esac
        fi
        mkdir zumcoin
        git clone -b master -q https://github.com/zumcoin/zumcoin zumcoin   >>build.log 2>&1 || _fail "Unable to clone git repository. Please see build.log for more information"
        cd zumcoin
    fi
}

_build_zumcoin() {
    _note "Building zumcoin from source (this might take a while)..."
    if [ -d build ]; then
...
_configure_linux() {
    if [ "$(awk -F= '/^NAME/{print $2}' /etc/os-release)" = "\"Ubuntu\"" ]; then
        ...
        _fail "Your OS version isn't supported by this installer. Please consider adding support for your OS to the project ('https://github.com/zumcoin')"
    fi
}
...
_configure_os() {
    ...
    case "${_unameOut}" in
        ...
        Darwin*)
            ...
            ;;
        *)
            _fail "This installer only runs on OSX 10.10+ and Ubuntu 16.04+. Please consider adding support for your OS to the project ('https://github.com/zumcoin')"
            ;;
    esac
    _note "Operating system configuration completed. You're halfway there!"
}

_note "Zumcoin Multi_Installer v1.0"
_colorize green "ZumCoin" && echo
...
_note "Look in 'zumcoin/build/src/' for the executible binaries. See 'https://github.com/zumcoin/zumcoin' for more project support. Cowabunga!"
