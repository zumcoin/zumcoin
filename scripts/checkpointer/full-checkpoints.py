...
def height():
    base_url = 'http://localhost:17935/getheight'
    resp = requests.get(base_url).json()
    if 'height' not in resp:
        print ('Unexpected response, make sure ZumCoind is running',
               resp)
        sys.exit(-1)
    else:
        return resp['height']
...
def rpc(method, params={}):
    base_url = 'http://localhost:17935/json_rpc'
    payload = {
        'jsonrpc': '2.0',
        'id': 'block_info',
        'method': method,
    ...
    if 'result' not in resp:
        print ('Unexpected response, make sure ZumCoind is running with block explorer enabled'
               , resp)
        sys.exit(-1)
    else:
        return resp['result']
