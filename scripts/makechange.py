...
Open the wallets with zum-serivce like so:

./zum-service -w walletA.wallet -p yourpass --rpc-password test --bind-port 17070
./zum-service -w walletB.wallet -p yourpass --rpc-password test --bind-port 17071
...
moveDecimal = 100000000  # ZUM has 8 decimals so 100000000 is the divide/multiply factor
minAmount = 100000000 * moveDecimal  # min number for amount to xfer
maxAmount = 50000000000000 * moveDecimal  # max number for amount to xfer
anonymity = 0
fee = 10000000 # atomic units, ZUM would be 0.10000000 as the tx network fee
